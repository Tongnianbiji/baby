...
export default class HospitalsView extends Presenter {
  render() {
    const {hospitalsList,searchValue,showInput} = this.state;
    return (
      <View className='school-vewport'>
        <View className='search-box'>
          <View className='inp-wrapper'>
            <AtInput className='inp' clear={true} placeholder='请输入医院关键字' value={searchValue} onChange={this.handleChange.bind(this)} placeholderClass='placehoder' confirmType='search' onConfirm={this.doSearch.bind(this)} />
        ...
        <View className='submit-btn-wrap'>
        ...
        </View>
          {
            <AtModal isOpened={showInput} onCancel={this.cancelInput.bind(this)} >
              ...
              {
                showInput &&
                <AtInput placeholder="请输入医院名称" value={searchValue} onChange={this.handleInput.bind(this)}></AtInput>
              }

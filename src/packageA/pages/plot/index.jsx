...
export default class SchoolsView extends Presenter {
  render() {
    const {plotList,searchValue,showInput,inputValue,showLoading,isToBottom,noResult} = this.state;
    return (
      <View className='school-vewport'>
        ...
        <View className='body-wrapper' style={{paddingBottom:plotList.length ? '295rpx' : '0'}}>
          {
            plotList.length ?
            <View>
              {
                plotList.map((item,index) => {
                  return (
                  <View className='item' onClick={this.selectItem.bind(this,item)}>{item.name}</View>
                  )
                }) 
              }
